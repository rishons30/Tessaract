...
import { addAircraft, addFlight } from "@/lib/api"
...
export function DataEntryPage() {
  ...
  const [preAssignmentText, setPreAssignmentText] = useState("")
  ...
  const handleFlightSubmit = async (e) => {
    ...
    if (
      ...
      !flightData.arr_time ||
      !flightData.subtype ||
      !flightData.min_seating_capacity ||
      !flightData.ground_time ||
    ...
    const flightPayload = {
      ...flightData,
      min_seating_capacity: Number.parseInt(flightData.min_seating_capacity),
      ground_time: Number.parseInt(flightData.ground_time),
      passengers: Number.parseInt(flightData.passengers),
      distance: Number.parseInt(flightData.distance),
    }
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: `Error adding flight: ${error.message}`,
      })
  ...
  const handleAircraftSubmit = async (e) => {
    ...
    // Parse pre-assignments
    let preAssignments = []
    if (preAssignmentText) {
      try {
        // Format: "12:00-14:00, 16:00-18:00"
        const blocks = preAssignmentText.split(",").map((block) => block.trim())

        preAssignments = blocks.map((block) => {
          const [start, end] = block.split("-").map((time) => time.trim())
          return {
            start: `2025-03-07 ${start}`,
            end: `2025-03-07 ${end}`,
          }
        })
      } catch (error) {
        setMessage({
          type: "error",
          text: "Invalid pre-assignment format. Use HH:MM-HH:MM format.",
        })
        return
      }
    }
    ...
    const aircraftPayload = {
      ...aircraftData,
      capacity: Number.parseInt(aircraftData.capacity),
      base_fuel: Number.parseFloat(aircraftData.base_fuel),
      efficiency: Number.parseFloat(aircraftData.efficiency),
    ...
    try {
      await addAircraft(aircraftPayload)
      ...
      setMessage({
        type: "error",
        text: `Error adding aircraft: ${error.message}`,
      })
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3">
                    <Input
                      ...
                      value={preAssignmentText}
                      onChange={(e) => setPreAssignmentText(e.target.value)}
