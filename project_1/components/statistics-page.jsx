        ...
        <TabsContent value="overview" className="space-y-6">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <StatCard
              ...
              value={isLoading ? "" : stats?.efficiency || 12.4}
              suffix="%"
              icon={BarChart}
              isLoading={isLoading}
            />
                  ...
                  <div className="h-[300px]">
                    <div className="flex h-full items-end gap-2 pb-6">
                      {["Jan", "Feb", "Mar"].map((month, i) => {
                        // Calculate a decreasing CO2 value for each month
                        const co2Value = baseline.total_co2 - i * 2000
                        const percentage = (co2Value / baseline.total_co2) * 100

                        return (
                          <div key={i} className="group relative flex h-full flex-1 flex-col justify-end">
                            <div className="relative h-full w-full transition-all" style={{ height: `${percentage}%` }}>
                              <div className="absolute bottom-0 w-full rounded-md bg-primary/70 p-1">
                                <div className="h-full w-full rounded-sm bg-primary"></div>
                              </div>
                            </div>
                            <span className="mt-2 text-center text-xs font-medium">{month}</span>
                ...
                {isLoading ? (
                  ...
                  <div className="space-y-8">
                    <div className="space-y-2 text-center">
                      ...
                      <Progress value={(76 / 95) * 100} className="h-2" />
                    </div>
